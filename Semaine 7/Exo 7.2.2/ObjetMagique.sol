...
contract objetMagique is ERC721{
    ...
    function doublon(uint _rarete, uint _type, uint _famille) private view returns (bool){
    ...
    function existe(uint _objectId) public view returns(bool){
        if(ObjectOwner[_objectId] != address(0)){
            return true;
        }
    }
